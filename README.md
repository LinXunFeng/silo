...
Engine complete and verified end to end from the command line. The Flutter macOS
app is scaffolded but not built yet.
...
| Flutter macOS UI | scaffolded only |
...
apps/silo_app        Flutter macOS shell (placeholder)
```
...
# Pull it in (defaults to Q4_K_M), then hard-link it into LM Studio
silo add Qwen/Qwen2.5-0.5B-Instruct-GGUF -v Q4_K_M --to lmstudio
...
# Forget a model, then reclaim the blobs nothing references
